// Render a logo candidate IN CONTEXT: dark app navbar, README hero, machine card.
// Usage: node docs/branding/render-context.mjs <svg>...
...
for (const f of process.argv.slice(2)) {
  const enc = `data:image/svg+xml;base64,${Buffer.from(readFileSync(f, "utf8")).toString("base64")}`;
  ...
  const out = resolve(dirname(f), basename(f).replace(/\.svg$/, ".context.png"));
