...
|----------|-----------|-----------|-----------|
| `rootfs.ext4.zst` | `rootfs/Dockerfile.openclaw` + agent + authproxy + scripts | `gs://openclawmachines/rootfs/` | Agent on host VMs (on boot / periodically) |
| `agent` | `backend/cmd/agent/` | `gs://openclawmachines/agent/` | Agent self-update on host VMs |
| `ocm` CLI | `cli/` | `gs://openclawmachines/cli/` | Developers / customers |
| `authproxy` | `backend/cmd/authproxy/` | Baked into rootfs | Not distributed separately |
| Backend image | `backend/Dockerfile` | Artifact Registry | Cloud Run |
| Frontend image | `frontend/Dockerfile` | Artifact Registry | Cloud Run |
...
```json
{
  ...
  "sha256": "e3b0c44...",
  "size_bytes": 52428800,
  "url": "gs://openclawmachines/agent/agent-abc1234-20260224T120000Z",
...
│                                                          │
│  4. Upload to GCS                                        │
│     ├─ rootfs: zstd compress → gs://openclawmachines/rootfs/
│     ├─ agent: → gs://openclawmachines/agent/             │
│     └─ CLI:   → gs://openclawmachines/cli/               │
│                                                          │
...
```bash
make deploy-backend    # Backend → Cloud Run (reads rootfs manifest URI from env)
make deploy-frontend   # Frontend → Cloud Run
...
Host VM boots
  ...
  │
  ├─ Agent checks gs://openclawmachines/agent/manifest.json
  ...
  └─ On first VM creation request:
      ├─ Agent checks gs://openclawmachines/rootfs/manifest.json
...
1. Fetch `gs://openclawmachines/agent/manifest.json`
...
gcloud run services update-traffic ocm-backend --to-revisions=PREVIOUS_REVISION=100
```
...
- [Rootfs Design](rootfs-design.md) — rootfs image contents and Dockerfile
- [Current Feature](CurrentFeature.md) — active development work
