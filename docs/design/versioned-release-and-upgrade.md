...
- Out of scope: data schema migration internals (covered elsewhere).
...
- Publish channel manifests to GCS:
  - `gs://YOUR-ARTIFACT-BUCKET/agent/manifest-{channel}.json`
  - `gs://YOUR-ARTIFACT-BUCKET/rootfs/manifest-{channel}.json`
  - `gs://YOUR-ARTIFACT-BUCKET/openclaw/manifest-{channel}.json`
