...
└────┬────┘     └──────┬──────┘     └──────┬──────┘     └─────┬─────┘     └──────┬──────┘     └────┬────┘
     │                 │                   │                  │                  │                 │
     │ GET https://acme.openclawmachines.com/my-bot/terminal/ws                                   │
     │────────────────>│                   │                  │                  │                 │
     │                 │                   │                  │                  │                 │
     │                 │ 1. Extract slugs  │                  │                  │                 │
     │                 │    account: acme  │                  │                  │                 │
     ...
     │                 │ GET https://{host}.openclawmachines.com/proxy/{machine_id}/terminal/ws   │
...
|------------------|----------|
| `openclawmachines.com` | → Frontend (Cloud Run) |
| `www.openclawmachines.com` | → Frontend (Cloud Run) |
| `api.openclawmachines.com` | → Backend API (Cloud Run) |
| `{account}.openclawmachines.com` | → Subdomain routing (below) |
...
Stateless API on Cloud Run (port 8080).
...
| **Agent Token** | GCP Secret Manager | VM metadata + Cloud Run env | Agent `bearerAuth` | Control Plane → Agent auth |
...
```
1. User Login
   Browser → Backend → JWT issued → Cookie set (domain=.openclawmachines.com)
...
wss://{account}.openclawmachines.com/{machine}/terminal/ws

# Browser (Playwright CDP WebSocket - port 3001)
wss://{account}.openclawmachines.com/{machine}/browser/ws

# Gateway (OpenClaw API - port 3000)
https://{account}.openclawmachines.com/{machine}/gateway/api/health
wss://{account}.openclawmachines.com/{machine}/gateway/ws

# Logs (SSE stream)
https://{account}.openclawmachines.com/{machine}/logs
```
...
│                       Cloudflare Tunnel                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  {host}.openclawmachines.com → cloudflared → localhost:9091        │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
└─────────┼───────────────────────────────────────────────────────────────────┘
          │
          ▼
...
- Cookie domain mismatch (must be `.openclawmachines.com`)
