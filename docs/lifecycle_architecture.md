...
- some headroom is needed for the host OS, tunnel, agent, paired browser VM overhead, and transient spikes
...
If paired browser VMs are common on a pool:
...
### 4. Paired browser VM is not modeled as a subresource

A browser VM is an independent, account-scoped Firecracker VM (headful Chromium, CDP on `:9222`, with a live view) that pairs 1:1 with a machine at runtime. Today its lifecycle is effectively nested inside the main VM lifecycle, but it does not have its own state model, cleanup contract, or compensating cleanup path. Pair-rule ownership is especially weak.
...
- paired browser VM cleanup removes pair rules on restart recovery
