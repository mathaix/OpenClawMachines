...
```
1. User visits openclawmachines.com
   │
...
3. User chooses auth method:
   ...
   │
4. CF Access sets CF_Authorization cookie (scoped to .openclawmachines.com)
...
1. User clicks "New Machine" in dashboard
   ...
   │  → Backend creates DNS: m-{slug}.openclawmachines.com → tunnel
...
2. Dashboard shows machine card:
   ...
   │  ┌─ my-agent ──────────────────────────────────────┐
   │  │  Status: ● Running                              │
   │  │  URL: m-my-agent.openclawmachines.com           │
   │  │                                                 │
   │  │  [Open Gateway] [Terminal] [SSH Info]            │
   │  └─────────────────────────────────────────────────┘
   │
3. User clicks "Open Gateway"
   │  → Browser navigates to https://m-my-agent.openclawmachines.com/
   │  → CF Access cookie already valid (same .openclawmachines.com domain)
   │  → NO second auth prompt
   │  → cloudflared tunnel → VM:18789 (gateway UI loads directly)
   │
4. User clicks "Terminal"
   │  → Browser opens https://m-my-agent.openclawmachines.com/terminal/
...
1. User clicks "SSH Info" in dashboard
   ...
   │  │  │   --hostname m-my-agent.openclawmachines.com       │ │
   │  │  └──────────────────────────────────────────[Copy]────┘ │
...
2. Server is accessible via tunnel path routing:
   │  https://m-my-agent.openclawmachines.com/port/8501/
...
1. CF Access session expires (configurable: 1 day / 7 days / 30 days)
   │
2. User's next request to any *.openclawmachines.com
...
```
*.openclawmachines.com          ← CF Access application scope
  │
  ├── openclawmachines.com       ← Frontend SPA + Backend API
  │     CF Access → CF_Authorization cookie (.openclawmachines.com)
  │     Backend reads Cf-Access-Jwt-Assertion header
  │
  ├── {account}.ocm.com          ← (legacy Worker route, removed after migration)
  │
  └── m-{slug}.openclawmachines.com   ← Per-VM tunnel
        Same CF_Authorization cookie valid here
        cloudflared tunnel → VM directly
        No Worker, no backend proxy, no agent proxy
```

**Key insight**: CF Access cookie is scoped to `.openclawmachines.com` (note leading dot). One authentication covers the main site AND all VM subdomains. No double-auth.
...
Type: Self-hosted
Domain: *.openclawmachines.com (wildcard)
         openclawmachines.com   (apex)
...
7. Frontend attaches token as X-Machine-Token header on all requests to m-{slug}.openclawmachines.com
```
...
**Problem**: `CF_Authorization` cookie is scoped to `.openclawmachines.com`. A malicious
...
```json
{
  ...
  "method": "GET",
  "path": "/terminal/ws",
  "origin": "https://m-my-agent.openclawmachines.com",
  "status": 101,
  "duration_ms": 2,
...
```json
{
  ...
  "machine_id": "m-abc123",
  "origin": "https://m-other-vm.openclawmachines.com",
...
9. **SSH password disabled**: `ssh -o PreferredAuthentications=password user@m-{slug}.openclawmachines.com` → **rejected** (only cert auth works)
...
- Add Access application for `*.openclawmachines.com` + `openclawmachines.com`
...
- Update `MachineWorkspace.tsx`: links point to `https://m-{slug}.openclawmachines.com/`
...
### Phase A
1. Visit `openclawmachines.com` → CF Access login page appears
...
2. Visit `m-{slug}.openclawmachines.com` with valid machine token → gateway UI loads
3. SSH: `cloudflared access ssh --hostname m-{slug}.openclawmachines.com` → shell
