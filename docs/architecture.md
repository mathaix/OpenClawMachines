...
Split-plane architecture derived from ClaraTeach. A stateless **Control Plane** API manages users, machines, and placement. **Worker Agents** on Host VMs run Firecracker MicroVMs — one per OpenClaw Machine. Multiple users' Machines share Host VMs via capacity-based bin-packing placement.
...
- [hardening.md](hardening.md) — Security hardening decisions
- [security_threat_model.md](security_threat_model.md) — Threat model for multi-provider hosts
...
│   │       └──► User Ports                                                  ││
│   │                                                                        ││
│   │   [Optional] Browser Companion VM (Chromium CDP :9222)                 ││
│   └────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────┘
```
...
flowchart TD
    ...
    %% ── Control Plane ───────────────────────────────────
    subgraph CP["Control Plane (Cloud Run)"]
        API["Go API :8080"]
...
| Decision | Rationale |
|----------|-----------|
| **Firecracker MicroVMs** | ~125ms boot, ~5MB overhead, KVM isolation. No K8s. Proven in ClaraTeach |
| **Split-plane (Control + Workers)** | Stateless Control Plane on Cloud Run. Worker Agents on dedicated hosts. Clean separation |
...
| **Managed Postgres (Neon)** | pgvector + tsvector, serverless, branching. Replaces MoltWorker's rsync |
| **LiteLLM per Host** | AI proxy runs on bridge IP (192.168.100.1:4000), accessible to all MicroVMs on that Host. BYOK + managed billing |

## Data Model

### ClaraTeach → OCM Mapping

| ClaraTeach | OCM | Change |
|------------|-----|--------|
| Instructor (user) | **User** | Cloudflare Access auth (Email OTP, Google, GitHub) |
| *(new)* | **Account** | Organization/workspace. Users belong to Accounts via membership |
| Workshop | **Machine** | A Machine is one OpenClaw instance. Account owns 1–N Machines |
| Workshop VM | **Host** | Shared across users. Multiple Machines per Host. Capacity-tracked |
| Session (seat) | *(removed)* | No seats — each Machine is the unit of deployment |
| Registration | *(removed)* | No student join flow — owner manages their Machine directly |
| LLM usage | **LLM usage** | Carried over, keyed by Machine instead of session |
...
CREATE TABLE machines (
    ...
    -- LLM
    budget_microcents BIGINT,              -- per-machine spend cap (migration 008)
...
-- ============================================================
-- LLM Usage (billing — tracked by LiteLLM proxy)
-- ============================================================
...
### Dashboard
- React + TypeScript + Vite app at `app.openclawmachines.com`
...
### Control Plane (Cloud Run — port 8080)
...
- **Billing Service** — Usage-based compute billing, BYOK + managed AI billing
...
**Responsibilities (carried from ClaraTeach):**
...
- Browser companion VM management (optional Chromium VM per machine for CDP access)
...
Each Machine runs in its own Firecracker MicroVM, optionally paired with a browser VM (see next section):
...
Carried from ClaraTeach's AI-Pass system:

- Runs on bridge IP `192.168.100.1:4000`, accessible to all MicroVMs on that Host
- Each Machine gets a virtual key (`sk-ocm-xxx`) if using managed billing
...
- **Budget limits**: Per-Machine spend cap, returns 402 when exhausted
...
- Encrypted data volume backups (`gs://openclawmachines/backups/`)

**Cloudflare Tunnels (Per-VM)**
- Each MicroVM gets its own isolated tunnel: `ocm-vm-{machine-slug}`
- DNS: `m-{slug}.openclawmachines.com` → tunnel → VM authproxy (port 8080)
...
```
RUN_MODE=api      → HTTP API + enqueue workflows (Cloud Run)
RUN_MODE=worker   → Execute workflows only (GCE spot instances)
RUN_MODE=""       → Both in one process (legacy, default)
```

**Why the split:** Cloud Run throttles CPU between requests, which kills DBOS executor goroutines. Workers need always-on CPU to poll queues and execute multi-minute workflows.
...
**Preemption handling:** Workers long-poll the GCE metadata server (`/instance/preempted?wait_for_change=true`). On preemption signal (30-second warning), they send SIGTERM to trigger graceful DBOS shutdown — in-flight steps checkpoint, and another worker resumes the workflow.
...
**Architecture:**
- **Agent handles all GCS I/O** — the agent has the data volume locally, compresses/encrypts/uploads directly. Cloud Run orchestrates and stores metadata only.
...
```
User (frontend)                Control Plane (Cloud Run)              Agent (host)                    GCS
     │                                │                                   │                            │
     │ POST /backups                  │                                   │                            │
     ├───────────────────────────────>│                                   │                            │
...
2. **Direct GCS** (fallback) — if the agent is unreachable, control plane streams directly from GCS. Only supports `format=ext4` (raw decrypted volume) since Cloud Run can't mount ext4 for tar.gz conversion.
...
## Technology Carried from ClaraTeach

| Package | What It Does | Changes for OCM |
|---------|-------------|-----------------|
| `internal/orchestrator/` | Firecracker VM lifecycle (create, destroy, network, rootfs, progress) | Key by `machine_id` instead of `workshop_id/seat_id`. Remove seat concept |
| `internal/agentapi/` | Worker Agent HTTP API (control + proxy) | Routes use `machine_id`. Add browser proxy, backup/restore endpoints |
| `internal/provisioner/` | GCP VM creation (nested virt, metadata injection) | Hosts are long-lived + shared, not per-workshop. Add capacity tracking |
...
| `internal/tunnel/` | Cloudflare Tunnel management | Route by Machine slug instead of workshop code |
...
### New Packages (not from ClaraTeach)
...
backend/
├── cmd/
│   ├── server/main.go          # Control Plane (Cloud Run)
...
| Component | Where | How |
|-----------|-------|-----|
| Control Plane | GCP Cloud Run | Docker (Go → alpine). Stateless, scales to zero |
| Dashboard | GCP Cloud Run | Docker (Vite → nginx). Static assets |
...
| Postgres | Neon | Serverless, `us-east-2` |
| AI Gateway | Per-Host VM | LiteLLM on bridge IP, started by agent |
| CI/CD | Google Cloud Build | `cloudbuild.yaml` |
| Build Artifacts | GCS (`gs://openclawmachines/`) | Rootfs, agent, CLI — see [Build Process](build-process.md) |

## Reference Notes

See [docs/reference-notes.md](reference-notes.md) for detailed research on OpenClaw internals, MoltWorker patterns, and Mission Control architecture that informed OCM's design.
...
sequenceDiagram
    ...
    alt No valid session
        ...
        CFAccess->>User: Set CF_Authorization cookie<br/>(.openclawmachines.com, 7-day TTL)
    end
...
|-------|-------|-------------|-------|
| **1. CF Access (Edge)** | `CF_Authorization` cookie | Cloudflare edge | User identity across all `*.openclawmachines.com` |
...
|-------|-----------|--------|----------|-------|
| **CF Session** | By Cloudflare Access after login | `CF_Authorization` cookie (`.openclawmachines.com`) | 7-day session | User identity across all domains |
...
- Create Access application for `*.openclawmachines.com` + `openclawmachines.com`
...
Each running MicroVM gets its own Cloudflare Tunnel with a dedicated subdomain: `m-{slug}.openclawmachines.com`.
...
sequenceDiagram
    ...
    Browser->>CFAccess: GET https://m-{slug}.openclawmachines.com/terminal/<br/>X-Machine-Token: {token}
...
The Cloudflare Worker (`worker.js`) still handles routing for the `{accountSlug}.openclawmachines.com` subdomain pattern, used for the dashboard iframe embedding of the gateway SPA. This path goes through the Agent proxy on the Host VM.
...
- **Production**: `https://m-{slug}.openclawmachines.com/{path}` (terminal, gateway)
- **SSH**: `ssh openclaw@ssh-{slug}.openclawmachines.com` (via `cloudflared access ssh`)
...
flowchart TD
    ...
    subgraph Agent["Worker Agent (proxy.go)"]
        ...
        P1 -->|"Rewrite"| S2["frame-ancestors 'none' →<br/>frame-ancestors 'self'<br/>openclawmachines.com<br/>*.openclawmachines.com"]
...
| `Content-Security-Policy: ... frame-ancestors 'none' ...` | **Rewritten** | `... frame-ancestors 'self' openclawmachines.com *.openclawmachines.com ...` |
...
2. The `frame-ancestors` allowlist restricts embedding to `openclawmachines.com` and its subdomains
