...
2. **DNS per VM** — Each tunnel gets a dedicated subdomain: `m-{slug}.openclawmachines.com`
...
Cloudflare Access (authentication)
    ↓
Cloudflare Tunnel (m-{slug}.openclawmachines.com)
    ↓
...
3. **Configure tunnel ingress rules:**
   ...
   //   "config": {
   //     "ingress": [
   //       { "hostname": "m-{slug}.openclawmachines.com", "service": "http://localhost:8080" },
...
4. **Create DNS record:**
   ...
   // Body: {
   //   "type": "CNAME",
   //   "name": "m-{slug}.openclawmachines.com",
