...
- **Firewall**: Port 9090 accessible from Cloud Run (for agent control API)
...
```bash
curl -X POST https://api.openclawmachines.com/api/admin/hosts/enrollment-tokens \
...
```bash
curl -sL https://api.openclawmachines.com/api/agent/install | sudo bash -s -- <YOUR_TOKEN>
```
...
```bash
# Test connectivity
curl -s https://api.openclawmachines.com/health
```
