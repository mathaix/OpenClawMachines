...
# provision-host.sh — Provision an Ubuntu 24.04 bare-metal host as an OpenClaw worker
#
# This script installs all system dependencies required before running the
# enrollment install script. It does NOT register the host with the control
# plane — that's done separately via the enrollment API.
#
# Usage:
...
KERNEL_GCS_PATH="gs://openclawmachines/vmlinux"
...
OCM_BROWSER_ROOTFS_GCS_MANIFEST="${OCM_BROWSER_ROOTFS_GCS_MANIFEST:-gs://openclawmachines/kernel-browser-rootfs/manifest.json}"
OCM_BROWSER_ROOTFS_VERSION="${OCM_BROWSER_ROOTFS_VERSION:-9a80fb2-20260411T202541Z}"
...
if [ ! -e /dev/kvm ]; then
    err "/dev/kvm not found. KVM is required for Firecracker. Ensure hardware virtualization is enabled."
fi
...
if [ "$OCM_STATE_REFLINK" != "yes" ] && [ -z "$OCM_BROWSER_XFS_DEVICE" ]; then
    log "WARNING: ${OCM_STATE_DIR} does not support reflink. New/unpinned Kernel Images browser rootfs canaries will be blocked unless OCM_ALLOW_KERNEL_BROWSER_FULL_COPY=1 is set."
fi
if [ -n "$OCM_BROWSER_XFS_DEVICE" ]; then
    ...
    if [ "$OCM_BROWSER_REFLINK" != "yes" ]; then
        log "WARNING: ${OCM_BROWSER_BASE} does not support reflink. Kernel Images browser rootfs canaries will be blocked unless OCM_ALLOW_KERNEL_BROWSER_FULL_COPY=1 is set."
    fi
else
...
if [ -f "$KERNEL_PATH" ]; then
    ...
    log "Downloading Firecracker-compatible kernel from GCS..."
    if gsutil cp "$KERNEL_GCS_PATH" "$KERNEL_PATH" 2>/dev/null; then
        ...
        log "WARNING: Could not download kernel from GCS. You'll need to:"
        log "  gsutil cp ${KERNEL_GCS_PATH} ${KERNEL_PATH}"
        log "  (Requires GCS credentials — will be available after enrollment)"
    fi
fi
...
echo "  2. Download agent binary:"
echo "     sudo gsutil cp gs://openclawmachines/agent/ocm-agent /usr/local/bin/ocm-agent"
